...
export function update(state: ECS6State, ecs6EntityId: EntityID, payload: any) {
  const transform = ecs7EnsureMutable(state, engine.baseComponents.Transform, ecs6EntityId)
...
export function remove(state: ECS6State, ecs6EntityId: EntityID) {
  engine.baseComponents.Transform.deleteFrom(ecs7EnsureEntity(state, ecs6EntityId))
}
